...
if (!isNull _target &&
    ...
    scopeName "SearchForSeat";
    {
        private _desiredRole = _x;
        {
            ...
            if ((_effectiveRole in ["driver", "gunner"]) && {unitIsUAV _target}) then {continue}; // Ignoring UAV Driver/Gunner
            if ((_effectiveRole == "driver") && {(getNumber (([_target] call CBA_fnc_getObjectConfig) >> "hasDriver")) == 0}) then {continue}; // Ignoring Non Driver (static weapons)
            ...
            if (_effectiveRole != _desiredRole) then {continue};
